(ns pinduoduo.main
  "Kotodama WASM entrypoint for the Pinduoduo clean-room actor (L4) — Clojure port.
  ...
  facts (`pinduoduo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pinduoduo")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "pinduodu_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "pinduodu_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "pinduodu_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"pinduoduo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pinduoduo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pinduoduo-compat/ready)
